...
/// </summary>
public class UserService(IRepository<WebAppDatabaseContext> repository, ILoginService loginService, IMailService mailService,WebAppDatabaseContext context,IHttpContextAccessor httpContextAccessor)
    : IUserService
{
    ...
    public async Task<ServiceResponse<AddressDto>> GetUserAddress(UserDTO userDto, CancellationToken cancellationToken = default)
    {
        var address = userDto.Address;
        return ServiceResponse.ForSuccess(address);
; 
    ...
    public async Task<ServiceResponse<LoginResponseDTO>> Login(LoginDTO login, CancellationToken cancellationToken = default)
    {
        ...
        var userBasket = await RetriveBasket(result.Name);
        var annonBasket = await RetriveBasket(httpContextAccessor.HttpContext?.Request.Cookies["userId"]);
        ...
        if (annonBasket != null)
        {
            ...
            annonBasket.UserId = result.Name;
        ...
        var user = new UserDTO
        {
            ...
            Name = result.Name,
            Role = result.Role,
            Basket = annonBasket != null ? annonBasket.MapBasketDto() : userBasket?.MapBasketDto()
    ...
    private async Task<Basket?> RetriveBasket(string userId)
    {
